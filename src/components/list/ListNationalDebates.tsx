...
import FormNationalDebate from "~/components/forms/FormNationalDebate";
import Modal from '~/components/modal/modal';
import { useSession } from "~/routes/plugin@auth";
import TableDebates from "~/components/table/TableDebates";
import { LuLayoutGrid, LuPlusCircle, LuTable } from "@qwikest/icons/lucide";
import { Tooltip } from "@qwik-ui/headless";
import { cn } from "@qwik-ui/utils";

import countries from "~/data/countries";
import { getFlagByName } from "~/utils";

interface ListNationalDebatesProps {
    title: string;
    debates: any[];
    tags: { id: string, name: string }[];
}

export default component$<ListNationalDebatesProps>(({ debates, tags, title }) => {
    console.log('ListNationalDebatesProps: ', debates)
            ...
            <div class="flex justify-between items-center">
                ...
                {debates.length > 0 && (
                    ...
                    >
                        ...
                        Start a New National Debate
                    </Button>
                )}
            </div>
            <h2 class="text-2xl">{getFlagByName(selectedCountry.value)} {selectedCountry.value}</h2>
