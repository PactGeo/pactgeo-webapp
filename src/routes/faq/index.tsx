...
import { SfAccordionItem } from 'qwik-storefront-ui';
...
export default component$(() => {
    ...
    const handleToggle = $((id: string, open: boolean) => {
        openedSignal.value = open
            ? [...openedSignal.value, id]
            : openedSignal.value.filter((item) => item !== id);
    });
        ...
        <div class="border border-neutral-200 rounded-md divide-y text-neutral-900">
            {accordionItems.map(({ id, summary, details }) => (
                <SfAccordionItem
                    ...
                    open={isOpen(id)}
                >
                    <p class="p-4 font-medium hover:bg-neutral-100 active:neutral-100">
                        {summary}
                    </p>
                    <div q:slot="detail">
